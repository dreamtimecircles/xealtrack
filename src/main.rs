...
fn load_config() -> AppConfig {
    ...
    log_message(&format!(
        "Configuration loaded successfully: OpenTrack={}:{}, Hotkey={}, Xreal={}",
        ip, port, hotkey, xreal_addr
    ));
...
fn send_handshake_a(device: &HidDevice) -> Result<(), hidapi::HidError> {
    ...
    let mut crc: u8 = 0;
    for i in packet.iter().take(64).skip(1) {
        crc = crc.wrapping_add(*i);
    }
    packet[64] = crc;
...
fn main() {
    ...
    let orientation: Arc<Mutex<OrientationState>> =
        Arc::new(Mutex::new(OrientationState::default()));

    if let Some(key) = parse_hotkey_key(&config.hotkey) {
        let recenter_state = Arc::clone(&orientation);
        key.bind(move || {
            if KeybdKey::LAltKey.is_pressed() && KeybdKey::LShiftKey.is_pressed() {
    ...
    let status_label_id = tray
        .inner_mut()
        .add_label_with_id("status")
        .expect("Failed to add status label");
    tray.inner_mut()
        .set_label("Initializing...", status_label_id)
        .expect("Failed to set status label");
    tray.add_label(&format!("Hotkey: LAlt + LShift + {}", config.hotkey))
        .expect("Failed to add hotkey label");
    ...
    if let Some(path) = target_device_path {
        log_message(&format!(
            "Opening targeted control interface (Interface {})...",
            TARGET_CONTROL_INTERFACE
        ));
        match api.open_path(&path) {
            Ok(device) => {
                ...
                log_message(&format!(
                    "Polling network layer. Awaiting connection to {}...",
                    &config.xreal_addr
                ));
                ...
                for attempt in 1..=15 {
                    ...
                    match XrealOne::new_with_addr(&config.xreal_addr) {
                        Ok(_) => {
                            ...
                            tray.inner_mut()
                                .set_label("Active", status_label_id)
                                .expect("Failed to set status label");
                            break;
                        }
                        Err(_) => {
                            log_message(&format!(
                                "Attempt {}: No response from target. Retrying...",
                                attempt
                            ));
                        }
                    };
            ...
            Err(_) => {
                log_message(
                    "Could not connect to USB HID endpoint. (Ensure you have sufficient permissions / udev rules).",
                );
        ...
        log_message(&format!(
            "Error: Could not locate Interface {} on your XREAL device.",
            TARGET_CONTROL_INTERFACE
        ));
        return;
    }
    ...
    thread::spawn(move || {
        ...
        log_message(&format!(
            "Connected to Xreal at {}",
            config_for_thread.xreal_addr
        ));
        ...
        while running_for_thread.load(Ordering::Relaxed) {
            match xreal.next() {
                Ok(imu) => {
                    ...
                    send_opentrack_packet(
                        &config_for_thread.ip,
                        config_for_thread.port,
                        yaw_deg,
                        roll_deg,
                        pitch_deg,
                    );
